...
if(isset($_GET['set_rank']) && isset($_GET['sub_id'])){
    ...
    header("Location: view_submissions.php?msg=rank_set"); exit();
}
...
<style>
    .section-card { background: #141920; border: 1px solid #1e2530; border-radius: 15px; overflow: hidden; }
    table { width: 100%; border-collapse: separate; border-spacing: 0; }
    thead th { background: #0d1117; color: var(--gold); font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1.5px; padding: 18px 20px; border-bottom: 2px solid #1e2530; }
    tbody td { padding: 16px 20px; border-bottom: 1px solid rgba(255,255,255,0.03); color: #94a3b8; font-size: 0.85rem; vertical-align: middle; }
    
    /* Luxury Position Badges */
    .rank-badge { 
        padding: 5px 12px; border-radius: 6px; font-size: 0.65rem; font-weight: 800; 
        display: inline-flex; align-items: center; gap: 6px; text-transform: uppercase;
    }
    .rank-1 { background: linear-gradient(45deg, #ffd700, #b8860b); color: #000; box-shadow: 0 0 15px rgba(255,215,0,0.2); }
    .rank-2 { background: linear-gradient(45deg, #e2e8f0, #94a3b8); color: #000; }
    .rank-3 { background: linear-gradient(45deg, #cd7f32, #8b4513); color: #fff; }

    .btn-action { padding: 8px 15px; border-radius: 8px; font-size: 0.75rem; font-weight: 700; transition: 0.3s; cursor: pointer; border: none; display: inline-flex; align-items: center; gap: 6px; text-decoration: none; }
    .btn-read { background: #1e2530; color: #fff; border: 1px solid #334155; }
    .btn-rank { background: rgba(201,168,76,0.1); color: var(--gold); border: 1px solid rgba(201,168,76,0.2); }
    .btn-rank:hover { background: var(--gold); color: #000; }
</style>
                    ...
                    <td>
                        <div style="display:flex; justify-content: flex-end; align-items:center; gap:12px;">
                            ...
                            <?php if(!empty($row['rank'])): ?>
                                <?php 
                                    $r = $row['rank'];
                                    $r_lbl = ($r==1)?'1st':(($r==2)?'2nd':'3rd');
                                ?>
                                <span class="rank-badge rank-<?php echo $r; ?>"><i class="fa-solid fa-crown"></i> <?php echo $r_lbl; ?> Place</span>
                                <button onclick="resetRank(<?php echo $row['id']; ?>)" style="background:none; border:none; color:#ef4444; cursor:pointer;"><i class="fa-solid fa-xmark"></i></button>
                            <?php else: ?>
...
function resetRank(subId) {
    Swal.fire({
        title: 'Reset Rank?', text: "Remove the winning status of this essay?", icon: 'warning',
        showCancelButton: true, background: '#141920', color: '#fff', confirmButtonColor: '#ef4444'
    }).then(r => { if(r.isConfirmed) window.location.href='view_submissions.php?reset_rank=1&sub_id='+subId; });
}

function confirmDelete(id) {
    Swal.fire({
        title: 'Delete?', icon: 'warning', showCancelButton: true, background: '#141920', color: '#fff', confirmButtonColor: '#ef4444'
